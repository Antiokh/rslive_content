...
## Что именно считается текстом статьи

Наличие точечного diff не ограничивает область проверки. Если MDX-файл изменён, `humanizer_russian` получает **всю head-версию статьи** и отдельно всю base-версию статьи.

Перед запуском из YAML front matter извлекаются только два редакторских поля:

1. `title`;
2. `description`, если оно есть.

Они добавляются к полному MDX body и проверяются теми же библиотеками, что основной текст. Остальные поля front matter (`keywords`, `ogSticker`, `sidebar`, `live` и любые другие служебные метаданные) в редакторский input не передаются.

Итоговый input имеет логическую форму:
...
PR-комментарий для каждой проверенной статьи показывает `Review scope`: наличие title/description, число символов полного body и итоговый размер текста, переданного в редактор. Поэтому на реальном PR можно проверить, что анализируется целая статья, даже когда изменена одна строка.

## Delta относительно base

Проверка отдельно прогоняет полные head- и base-версии страницы. В комментарий попадают только новые normalized findings текущего PR. Сигнатура учитывает библиотеку, правило, phenomenon, project class, automation level, verdict и нормализованный фрагмент.
...
Это означает две разные вещи одновременно:

- **контекст анализа полный** — title, description и весь body;
- **вывод PR локальный** — показываются только новые findings относительно base.

Комментарий показывает:

- область реально проверенного текста (`Review scope`);
...
6. область проверки равна `whole_article`;
7. сырой front matter исключается;
8. контрольная проблема в `title` ловится правилом `ILY-M01`;
9. та же проблема в `description` ловится правилом `ILY-M01`;
10. та же строка, помещённая только в служебное поле front matter, не даёт `ILY-M01`;
11. body входит в редакторский input целиком.

Если контракт `humanizer_russian` или MDX-подготовки изменится несовместимо, workflow упадёт до публикации PR-комментария.
