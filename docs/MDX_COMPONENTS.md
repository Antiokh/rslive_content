...
- Старые типы DokuWiki/Bootstrap `primary`, `success`, `warning`, `info`, `question` не переносите автоматически: выберите ближайший поддерживаемый смысл.
...
- Компонент генерирует QR-код; не добавляйте отдельный внешний QR без необходимости.
...
```mdx
<ContentInclude page="move:visa" section="таможенный-контроль" noHeader />
```

Правила:

- `page` использует DokuWiki-подобное namespace-обозначение для поиска файла: `move:visa` → `src/content/docs/move/visa/index.mdx`.
- Перед использованием проверьте существование исходной страницы.
- `section` должен соответствовать реально распознаваемому заголовку исходной страницы.
...
- Для юридически критичного текста предпочтительно проверить собранную страницу, а не полагаться только на исходный MDX.
...
Можно передать ID или полный YouTube URL. По умолчанию используется privacy-enhanced домен.
...
Не используйте компонент как универсальный график. Для другого набора данных нужен отдельный компонент или изменение реализации в `Antiokh/rslive.ru` с обновлением документации.
...
- Для внешнего JSON/API можно использовать `src`, но сначала проверьте формат ответа по исходнику компонента.
...
Компонент содержит реестр известных схем и их колонок. Перед использованием проверьте, что `schema` существует в текущем исходнике. Не придумывайте имя схемы по аналогии.
