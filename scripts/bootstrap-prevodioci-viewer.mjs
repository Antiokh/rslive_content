...
const mid = '1qkp5VjmSJBjdaRRnxEtv0gZEaL2Dpk4';
const bounds = [18.75, 42.2, 23.05, 46.2];
const output = path.resolve('map-data/snapshots/google-mymaps');
const headers = {
  accept: '*/*',
  'user-agent': 'Mozilla/5.0 RSLive LKG bootstrap',
};
...
if (!/<kml\b/i.test(kmlResponse.text)) throw new Error('Translator export is not KML.');
const placemarks = kmlResponse.text.match(/<Placemark\b[\s\S]*?<\/Placemark>/gi) || [];
if (!placemarks.length) throw new Error('Translator KML contains no Placemark.');
...
if (coordinatesByName.size !== placemarks.length) {
  throw new Error(`Viewer/KML feature count mismatch: viewer=${coordinatesByName.size}, KML=${placemarks.length}`);
}
...
const features = placemarks.map((placemark) => {
  ...
  return {
    ...
    properties: { name, description, ...(address ? { address } : {}) },
  };
});
...
const geojson = {
  ...
  properties: {
    sourceKind: 'google-mymaps',
    sourceId,
    upstreamUrl: `https://www.google.com/maps/d/viewer?mid=${mid}`,
    acquisition: 'google-kml-plus-viewer-coordinates',
    snapshotAt,
...
const metadata = {
  ...
  viewerUrl,
  snapshotAt,
  contentType: kmlResponse.contentType,
  bytes: kmlResponse.bytes.byteLength,
  sha256: createHash('sha256').update(kmlResponse.bytes).digest('hex'),
  counts: {
    placemarks: placemarks.length,
    folders: (kmlResponse.text.match(/<Folder\b/gi) || []).length,
    points: 0,
    lineStrings: 0,
    polygons: 0,
    multiGeometries: 0,
    geometries: features.length,
    viewerPoints: coordinatesByName.size,
  },
  dataFile: `${mid}.kml`,
};
...
console.log(`[maps] translator LKG: ${features.length} point(s), ${kmlResponse.bytes.byteLength} KML bytes.`);
