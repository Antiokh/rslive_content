...
function audit(file, text) {
  ...
  const isPlayground = relative.includes('/playground/');
  ...
  if (!isPlayground) {
    checks.push(['DokuWiki internal link', /\[\[[^\]]+\]\]/]);
    checks.push(['DokuWiki media/include', /\{\{[^}]+\}\}/]);
  }
...
for (const file of files) {
  ...
  if (output !== source) {
    changed.push(path.relative(process.cwd(), file).replaceAll('\\', '/'));
